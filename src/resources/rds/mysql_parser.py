...
_SLOW_USERHOST_CRE = re.compile(r"#\s+User@Host:\s+(?:([\w\d\-\.]+))?\s*\[\s*([\w\d\-\.]+)\s*\]\s*@\s*([\w\d\.\-]*)\s*\[\s*([\d.]*)\s*\]\s*(?:Id\:\s*(\d+)?\s*)?")

_SLOW_STATS_CRE = re.compile(r"#\sQuery_time:\s(\d*\.\d{1,6})\s*Lock_time:\s(\d*\.\d{1,6})\s*Rows_sent:\s(\d*)\s*Rows_examined:\s(\d*)\s*(Rows_affected:\s(\d*)\s*Rows_read:\s(\d*)){0,1}")

_THREAD_INFO_CRE = re.compile(r"#\sThread_id:\s(\d*)\s*Schema:\s(.*)")
...
class SlowQueryLog(LogParserBase):
    ...
    def _parse_statistics(self, line, entry):
        ...
        entry['query_time'] = float(result[0]) if result[0] else 0
        entry['lock_time'] = float(result[1]) if result[1] else 0
        entry['rows_sent'] = int(result[2]) if result[2] else 0
        entry['rows_examined'] = int(result[3]) if result[3] else 0
        entry['rows_affected'] = int(result[4]) if result[4] else 0
        entry['rows_read'] = int(result[5]) if result[5] else 0
    ...
    def _parse_entry(self):
        ...
        5. A line setting the timestamp, insert_id, and last_insert_id
           session variables
        6. An optional administrator command line "# administrator command"
